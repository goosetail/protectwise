/**
 * Client-side generation of markup from templates.  
 * Templates are written using a classic ASP/JScript syntax.
 */
...
( function( $ ) {
	...
	$.TemplateManager = (function() {
		...
		var templateRoot = '',
			...
			loadedTemplates = {},
			loadedTemplatePaths = {},
			locale = { text: function( val ) { return val; } };
		...
		* and turns them into individual templates.
		*
		* @param {Object} oArgs
		*/
		function loadTemplates(oArgs) {
			...
			var paths = oArgs.paths ? [].concat(oArgs.paths) : [],
				templates = oArgs.templates ? [].concat(oArgs.templates) : [],
				pathLen = paths.length,
				templateLen = templates.length,
				total = pathLen + templateLen,
				totalLoaded = 0,
				self = this,
				path, template;
			...
			for (var i = 0; i < pathLen; i++) {
				...
				if (!loadedTemplatePaths[path]) { // NOT REQUESTED					
					...
					loadedTemplatePaths[path] = $.get(path, templateLoaded);
			...
			// load all the template strings (for script elements)
			for (var k = 0; k < templateLen; k++) {
				template = $(templates[k]);
				templateLoaded(template.html());
			}
		...
		*/
		function registerTemplate(name, template) {
			...
			function buildTemplate(str) {
				...
				try {
					return new Function("data, iterator, locale", [
						"var p=[],",
	...
	$.extend({
		fillTemplate: function(name, obj, context) {
			return $($.TemplateManager.fillTemplate(name, obj, context));
		},
		loadTemplates: function(args) {
			return $.TemplateManager.loadTemplates(args);
		}
	});
	
	$.extend($.fn, {
		fillTemplate: function(name, obj, context) {
			this.html($.fillTemplate(name, obj, context));
			return this;
		},
		appendTemplate: function(name, obj, context) {
			this.append($.fillTemplate(name, obj, context));
			return this;
		}
	});
